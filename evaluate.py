...
test_dataset = None
with open('data/test_dataset', 'rb') as f:
...
model.load_state_dict(torch.load("./model/first_try"))  # ,  map_location=torch.device('cpu')
# print(model)
model.to(device)
model.eval()
result_list = []
...
with torch.no_grad():
    for batch in test_loader:
        ...
        print("finished batch %d, loss: %f" % (i, outputs[0]))
        i = i+1

F1, acc = cal_F1(result_list, label_list)
print("F1: %f ; acc: %f" % (F1, acc))  # /927
